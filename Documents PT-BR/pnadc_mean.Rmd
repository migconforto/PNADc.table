...
* **export**:
<br/> Formato em que deseja export a tabela gerada. Pode export para "html", "pdf", "png", "csv" e "rtf". Caso queira retornar um data frame em espaço R utilizar "df". Caso não tenha sido preenchido retorna uma gt_table em espaço R. Deve ser uma string (vir entre " ").
...
# Exemplos
<div style="text-align: justify">
  Nesse exemplo vamos criar uma tabela da média do rendimento bruto mensal (V403312) separado por unidade da federação (UF) de 2019.1 contando que o usuário não utilizou a função _pnadc_download_ para fazer o download dos arquivos e também não deseja export a tabela.
```{r}
...
#Como o path já foi especificado pela função pnadc_download, basta apenas colocar o valor lógico True no argumento path da função pnadc_mean.
...
#Atribuímos ao objeto "table_mean_pnadc_2018_2" o Data Frame da função, dessa forma ele ficará salvo para poder ser utilizado posteriormente.
