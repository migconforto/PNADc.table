...
* **Export**:
Formato em que deseja export a tabela gerada. Pode export para "html", "pdf", "png", "csv" e "rtf". Caso queira retornar um data frame em espaço R utilizar "df". Caso não tenha sido preenchido retorna uma gt_table em espaço R. Deve ser uma string (vir entre " ").
...
# Exemplos:
<div style="text-align: justify">
Nesse exemplo vamos criar uma tabela da mean do rendimento bruto mensal (V403312) separado por faixa etária de 5 em 5 anos da PNADc de 2019.1 calculationndo que o usuário não utilizou a função _pnadc_download_ para fazer o download dos arquivos e também não deseja export a tabela.
...
#O group será o próprio filtro já que estamos trabalhando apenas com um filtro.
...
<div style="text-align: justify">
Agora vamos **export** as tabelas anteriores para uma pasta pré selecionada

```{r}
pnadc_tracks(~V403312, ~V2009+UF, 2019, 1, calculation = "percentage", path = "C:/Dados_PNADc", group = ~V2009, cluster = c(0, 100, 5), export = "pdf")
#Como o path foi selecionado, a função irá baixar e criar o design diretamente no diretório escolhido
